...
# kill the session for a selected entry (session name, or project path)
kill_session() {
    local target="$1"
    [[ "$target" == /* ]] && target="$(basename "$target" | tr . _)"
    ...
    other=$(tmux list-sessions -F '#S' 2>/dev/null | grep -vFx "$target" | head -1)

    if [[ -n "$current" && "$current" == "$target" && -n "$other" ]]; then
...
find_dirs() {
    ...
    # each directory is printed as "name|parent|path"
...
# format "name|parent|path" project entries and "[TMUX] name" session lines
# as "name  [parent]" / "name  [TMUX]", right-padding names so they align.
# the payload (project path, or TMUX:<session>) is appended after a tab so
# it is hidden from fzf and parsed back after selection.
format_dirs() {
    awk -F'|' '
        NF == 3 {
...
if [[ ! -z $session_cmd ]]; then
    ...
    while true; do
        result=$(find_dirs | sort -t'|' -k2,2 -k1,1 | format_dirs | \
            fzf --expect=ctrl-x --prompt="Projects > " --height=100% --reverse --nth 1 --with-nth 1 --delimiter '\t')
        key=$(printf '%s\n' "$result" | sed -n '1p')
        selected=$(printf '%s\n' "$result" | sed -n '2p')

        # no selection (ESC) → exit
        if [[ -z $selected ]]; then
            exit 0
        fi

        # project and session entries carry their payload after a tab
        if [[ "$selected" == *$'\t'* ]]; then
            IFS=$'\t' read -r _ selected <<< "$selected"
        fi

        # session entries are tagged TMUX:<name>
        if [[ "$selected" == TMUX:* ]]; then
            selected="${selected#TMUX:}"
        fi

        # Ctrl+X kills and stays in the picker
        if [[ "$key" == "ctrl-x" ]]; then
